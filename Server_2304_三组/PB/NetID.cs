using System.Collections;
using System.Collections.Generic;

public class NetID
{
    public static int C_To_S_GetShop = 1001;
    public static int S_To_C_GetShop = 1002;
    public static int C_To_S_GetBag = 1003;
    public static int S_To_C_GetBag = 1004;
    public static int C_To_S_BayShop = 1005;
    public static int S_To_C_BayShop = 1006;
    public static int C_To_S_GetPlayer = 1007;
    public static int S_To_C_GetPlayer = 1008;
    public static int C_To_S_Player_Res = 1009;
    public static int S_To_C_Player_Login = 1010;
    public static int S_To_C_PlayerInfo = 1011;
    public static int S_To_C_RedPoint_Msg = 1012;
    
    public static int C_To_S_PlayerOperation = 10001;
    public static int S_To_C_PlayerOperation = 10002;
    public static int C_To_S_Disconnect = 10003;
    public static int S_To_C_Disconnect = 10004;
    public static int C_To_S_Bullet = 10005;
    public static int S_To_C_Bullet = 10006;
    public static int C_To_S_ReomveBullet = 10007;
    public static int S_To_C_ReomveBullet = 10008;
    public static int C_To_S_PlayerAtk = 10009;
    public static int S_To_C_PlayerHp = 10010;
    public static int C_To_S_PlayerAlive = 10011;
    public static int C_To_S_PlayerAnimator = 10012;
    public static int S_To_C_PlayerAnimator = 10013;
    public static int C_To_S_ExitGame = 10014;
    public static int S_To_C_ExitGame = 10015;
    public static int C_To_S_Gold = 10015;
    public static int S_To_C_Gold = 10015;
}
