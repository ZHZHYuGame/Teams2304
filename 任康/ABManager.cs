...
public class ABManager : MonoBehaviour
{
    ...
    { 
        if (Input.GetMouseButtonDown(0))
        {
            if (m_obj == null)
            {
                string mPath = Application.streamingAssetsPath + "/AssetBundle/capsule.u3d";
                AssetBundle aBundle =  AssetBundle.LoadFromFile(mPath);
                //通过资源包找到里面存在的物体
                m_obj = aBundle.LoadAsset<GameObject>("capsule");
            }
    ...
    public void Init()
    {
        abPath = Path.Combine(Application.streamingAssetsPath + "/AssetBundle");
        InitDependence();
    }
    /// <summary>
    /// 初始化资源包的依赖关系
    ...
    public void InitDependence()
    {
        if (allDependDict == null)
        {
            ...
            // string path = Path.Combine(abPath, "cube");
            string path = $"{abPath}/cube.u3d";
            //加载资源包
