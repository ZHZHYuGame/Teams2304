...
function BagModel:InitData(baglist)
    for i = 0, baglist.Count - 1, 1 do
        ...
        if data.Type == "资源" then
            self.bagDataDict[0][bagData.id] = bagData
        end
        if data.Type == "加速" then
            self.bagDataDict[1][bagData.id] = bagData
        end
        if data.Type == "增益" then
            self.bagDataDict[2][bagData.id] = bagData
        end

        if data.Type == "装备" then
            self.bagDataDict[3][bagData.id] = bagData
        end
        if data.Type == "其他" then
            self.bagDataDict[4][bagData.id] = bagData
        end
    end
end
