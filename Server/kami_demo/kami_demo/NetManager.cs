...
public class NetManager:Singleton<NetManager>
{
    ...
    private void AcceptHandle(IAsyncResult ar)
    {
        ...
        Console.WriteLine($"IP: {ip.Address}  端口号: {ip.Port}");
            ...
            if (dataLen > 0)
            {
                ...
                while (myStream.Length >= 2)
                {
                    ...
                    if (myStream.Length >= allLen)
                    {
                        ...
                        MessageControl.GetInstance().Dispach(netID, descByte, st);
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    public Socket Get_Socket(int id)
    {
    ...
    public int Get_PlayerId(Socket st)
    {
        foreach (var c in clientsList)
        {
            if (c.st == st)
